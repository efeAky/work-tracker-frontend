      ...
      >
        ...
        <div className="flex items-center gap-3 self-start sm:self-auto">
          <Link
            ...
            className="px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-black uppercase tracking-widest rounded-xl transition-all active:scale-95 shadow-lg shadow-indigo-100"
          >
            + Create User
          </Link>

          <div className="flex items-center gap-3 bg-white border border-slate-100 rounded-2xl px-6 py-4 shadow-xl shadow-slate-200/50">
      ...
      >
        ...
        {loading ? (
          ...
          <div className="bg-white rounded-[40px] border border-slate-100 p-20 text-center shadow-inner">
            <p className="text-slate-400 font-bold uppercase tracking-widest text-xs">No users yet.</p>
          </div>
        ) : (
            ...
            {recentUsers.map((user, index) => (
              <motion.div
                ...
                className={`group flex flex-col sm:flex-row items-start sm:items-center justify-between px-6 py-5 hover:bg-slate-50 transition-all ${
                  index !== recentUsers.length - 1 ? "border-b border-slate-50" : ""
                }`}
              >
                <div className="flex items-center gap-4 mb-4 sm:mb-0 w-full sm:w-auto">
                  ...
                  <div className="min-w-0">
                    <p className="font-black text-slate-900 text-base tracking-tight truncate">{user.fullname}</p>
